...
$(document).ready(function() {
	...
	$("#login").click(function() {
		...
		if(username.length > 20 || username.length === 0 || password.length > 20 || password.length < 8) {
		...
		mqant.init({
		    ...
		    onSuccess:function() {
				...
				mqant.request(topic, {
					...
					mqant.request("Test/HD_FindUserByName",{
						"userName": username
					}, function(data) {
						var message=JSON.parse(data.payloadString)
						if(message.Error!="") {
		...
		mqant.init({
			...
			onSuccess:function() {
				...
				mqant.request(topic, {
					...
					// mqant.request("Chat/HD_FindUserByName",{
					// 	"userName": username
					// }, function(data) {
					// 	var message=JSON.parse(data.payloadString)
					// 	if(message.Error!="") {
	...
	$("#modify").click(function() {
		username = $("#loginUser").attr("value");
		var useSSL = 'https:' == document.location.protocol ? true :false ;
		mqant.request("Test/HD_ModifyUser",{
			"userName": username,
			"address": "wh",
			"tel":1514142444,
